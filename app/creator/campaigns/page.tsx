  ...
  return (
    <DashboardLayout>
      <div className="mb-6">
        ...
        <p className="text-zinc-500 mt-1">Browse and join campaigns to start earning</p>
      </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((campaign) => (
            <div key={campaign.id} className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 flex flex-col">
              ...
              <p className="text-sm text-zinc-500 mb-4 flex-1 line-clamp-2">{campaign.description}</p>
              ...
              <div className="space-y-2 text-sm mb-4">
                ...
                {campaign.rates.perView && (
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-green-400">₦{campaign.rates.perView}</span>
                  </div>
                )}
                {campaign.rates.perLike && (
                  <div className="flex justify-between">
                    <span className="text-zinc-500">Per like</span>
                    <span className="font-medium text-green-400">₦{campaign.rates.perLike}</span>
                  </div>
                )}
                {campaign.rates.perComment && (
                  <div className="flex justify-between">
                    <span className="text-zinc-500">Per comment</span>
                    <span className="font-medium text-green-400">₦{campaign.rates.perComment}</span>
                  </div>
                )}
                {campaign.rates.fixedRate && (
                  <div className="flex justify-between">
                    <span className="text-zinc-500">Fixed rate</span>
                    <span className="font-medium text-green-400">{formatCurrency(campaign.rates.fixedRate)}</span>
                  </div>
                )}
                <div className="flex justify-between">
                  <span className="text-zinc-500">Platforms</span>
                  <span className="font-medium text-white">{campaign.platforms.join(', ')}</span>
                </div>
                {campaign.endDate && (
                  <div className="flex justify-between">
                    <span className="text-zinc-500">Ends</span>
                    <span className="font-medium text-white">{formatDate(campaign.endDate)}</span>
                  </div>
                )}
              </div>
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-md p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-bold text-white">Submit Content</h2>
              <button onClick={() => setSelectedCampaign(null)} className="text-zinc-500 hover:text-zinc-300">
                <X size={20} />
              </button>
            </div>

            <p className="text-sm text-zinc-500 mb-4">Campaign: <strong className="text-white">{selectedCampaign.title}</strong></p>
            <p className="text-sm text-zinc-300 mb-1 font-medium">Requirements:</p>
            <p className="text-sm text-zinc-500 mb-5">{selectedCampaign.requirements}</p>

            {submitSuccess ? (
              <div className="text-center py-4">
                <p className="text-green-400 font-semibold">Submission successful!</p>
                <p className="text-sm text-zinc-500 mt-1">Your submission is pending review.</p>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-zinc-300 mb-1">Content Link</label>
                  <div className="relative">
                    <ExternalLink size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-500" />
                    <input
                      type="url"
                      value={link}
                ...
                <div className="flex gap-3 pt-2">
                  <button type="button" onClick={() => setSelectedCampaign(null)} className="flex-1 py-2.5 rounded-xl border border-zinc-700 text-sm font-medium text-zinc-300 hover:bg-zinc-800">
                    Cancel
                  </button>
                  <button type="submit" disabled={submitting} className="flex-1 py-2.5 rounded-xl bg-green-500 hover:bg-green-400 text-black text-sm font-bold disabled:opacity-60">
